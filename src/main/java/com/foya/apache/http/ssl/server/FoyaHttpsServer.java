...
public class FoyaHttpsServer {

	private static final Logger mLogger = LoggerFactory.getLogger(FoyaHttpsServer.class);
	...
	public FoyaHttpsServer() throws IOException {
		super();
	...
	public void start() throws IOException, KeyManagementException, UnrecoverableKeyException, NoSuchAlgorithmException,
			KeyStoreException, CertificateException {
		Asserts.check(this.server == null, "Server already running");

		String keystore = ClassPathUtils.toFullyQualifiedPath(FoyaHttpsServer.class, "test.keystore");
		mLogger.debug("[LOG][keystore]" + keystore);
		final URL resource1 = getClass().getResource("/" + keystore);
		final String storePassword = "nopassword";
		final String keyPassword = "nopassword";
		final SSLContext sslContext = SSLContextBuilder.create()
				.loadKeyMaterial(resource1, storePassword.toCharArray(), keyPassword.toCharArray()).build();
		...
		this.server = ServerBootstrap.bootstrap()
				...
				.setServerInfo("FOYA-SERVER/1.1").setConnectionFactory(new LoggingConnFactory())
	...
	static class SimpleExceptionLogger implements ExceptionLogger {
		...
		private final Log log = LogFactory.getLog(FoyaHttpsServer.class);
