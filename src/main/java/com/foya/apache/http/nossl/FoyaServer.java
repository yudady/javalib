...
public class FoyaServer {
	...
	public FoyaServer(String ip, int port) throws IOException {
		super();
		this.ip = ip;
		this.port = port;
	...
	public void start() throws IOException {
		...
		this.server = ServerBootstrap.bootstrap().setListenerPort(port).setLocalAddress(InetAddress.getByName(ip)).setSocketConfig(SocketConfig.custom().setSoTimeout(this.timeout).build())
				.setServerInfo("TEST-SERVER/1.1").setConnectionFactory(new LoggingConnFactory()).setExceptionLogger(new SimpleExceptionLogger())
	...
	static class SimpleExceptionLogger implements ExceptionLogger {
		...
		private final Log log = LogFactory.getLog(FoyaServer.class);
